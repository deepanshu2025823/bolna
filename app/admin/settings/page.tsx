...
function AdminSettingsContent() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (data.data.settings?.custom_domain) {
            setDomainData({ custom_domain: data.data.settings.custom_domain });
  ...
  const unlockDomain = () => {
    if (confirm("Are you sure you want to unlock and change the master portal domain? Clients relying on this might face issues if changed unexpectedly.")) {
    ...
    <div className="space-y-6 sm:space-y-8 font-sans animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
        ...
        <div className="flex-1 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {activeTab === 'domain' && (
            <div className={`p-6 sm:p-10 animate-fade-in ${statusMsg ? 'pt-2' : ''}`}>
              <h3 className="text-xl font-bold text-slate-900 mb-2">Host Portal via iFrame</h3>
              <p className="text-sm text-slate-500 mb-6 font-medium">Generate an iFrame to embed this administrative portal on your own external website.</p>
              
              <form onSubmit={(e) => handleSave(e, 'domain')} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">Where will you host this? (Your Custom Domain)</label>
                  <div className="flex gap-3">
                    <input 
                      ...
                      placeholder="e.g. admin.myagency.com" 
                  ...
                  {domainLocked ? (
                    <div className="flex items-center mt-2">
                      <p className="text-xs text-emerald-600 font-bold flex items-center">
                        ...
                        Domain locked as the master portal origin.
                      </p>
                      <button type="button" onClick={unlockDomain} className="text-xs ml-3 text-blue-600 hover:text-blue-800 underline font-semibold">
                        Unlock / Edit
                      </button>
                    </div>
                  ) : (
                    <p className="text-xs text-slate-500 mt-2 font-medium">Note: Setting this helps track the origin of the portal.</p>
                  )}
                </div>

                {domainLocked && (
                  <div className="bg-blue-50 border border-blue-100 p-5 rounded-xl animate-fade-in mt-6">
                    <h4 className="font-bold text-blue-900 mb-2">Your iFrame HTML Code</h4>
                    <p className="text-sm text-blue-800 mb-4 leading-relaxed">
                      Copy and paste this code into the <code>index.html</code> of your domain (<strong>{domainData.custom_domain}</strong>) to display your portal seamlessly.
                    </p>
