...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      <aside 
        className={`fixed lg:static inset-y-0 left-0 z-30 w-[280px] bg-[#0B1121] border-r border-slate-800/60 text-white transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="h-20 flex items-center px-6 border-b border-slate-800/60">
          ...
          <span className="text-xl font-extrabold tracking-tight text-white">
            AI Portal <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Pro</span>
          </span>
        </div>
          ...
          <nav className="space-y-1.5">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <svg 
                    className={`w-5 h-5 mr-3 transition-transform duration-200 ${isActive ? 'text-white scale-110' : 'text-slate-500 group-hover:text-white'}`} 
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
        ...
        <div className="p-4 border-t border-slate-800/60 m-4 rounded-2xl bg-slate-800/30">
          <Link 
            href="/login" 
            className="group flex items-center px-4 py-2.5 text-sm font-semibold rounded-xl text-slate-300 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200"
          >
            <div className="w-8 h-8 rounded-full bg-slate-700/50 flex items-center justify-center mr-3 group-hover:bg-red-500/20 transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header 
          className={`sticky top-0 z-20 h-20 transition-all duration-200 flex items-center justify-between px-4 sm:px-6 lg:px-10 ${
            scrolled ? 'bg-white/80 backdrop-blur-xl border-b border-gray-200/80 shadow-sm' : 'bg-transparent border-b border-transparent'
          }`}
        >
          <div className="flex items-center">
            <button 
              onClick={() => setIsSidebarOpen(true)}
              className="lg:hidden text-slate-500 hover:text-slate-800 hover:bg-slate-100 p-2.5 rounded-xl mr-3 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50"
            >
          ...
          <div className="flex items-center space-x-3 sm:space-x-5">
             <button className="text-slate-400 hover:text-blue-600 hover:bg-blue-50 relative p-2.5 rounded-full transition-all duration-200">
                <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
                <span className="absolute top-2 right-2 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white shadow-sm"></span>
             </button>
             
             <div className="flex items-center cursor-pointer hover:bg-white p-1 pr-3 rounded-full border border-transparent hover:border-gray-200 hover:shadow-sm transition-all duration-200">
               ...
               <div className="hidden md:block ml-3">
                 ...
                 <p className="text-xs font-medium text-slate-500">Master Account</p>
               </div>
